# How to Integrate Signal Sections into MusicScope™ Dashboard
...
3. **Signal modules created** - All 3 files compile successfully:
   - `tools/notebook_helpers/musicscope_momentum.py`
   - `tools/notebook_helpers/musicscope_sentiment.py`
   - `tools/notebook_helpers/musicscope_performance.py`
...
```python
# ============================================================================
# SECTION 3: ❤️ SENTIMENT INTELLIGENCE
# ============================================================================
...
### **Import Error: `tools.notebook_helpers`**

**Problem:** `ModuleNotFoundError: No module named 'tools'`

**Solution:** Ensure `tools/notebook_helpers/musicscope_design_system.py` exists and the project root is on `sys.path`.
...
**Solution:** The Signal modules use `resolve_artist_column()` to auto-detect artist columns. If you see this error, check that your data has at least one of: `artist_name`, `artist`, `channel_title`, `uploader`.
...
```bash
git add tools/notebook_helpers/musicscope_*.py
git add "notebooks/MusicScope™_Professional_Dashboard.ipynb"
git commit -m "feat(notebooks): integrate Signal sections with modular architecture

- Created 3 modular Signal files (momentum, sentiment, performance)
- Hybrid architecture: design system + existing helpers
- Replaced old sections with clean function calls
- All charts render with Signal business framing
