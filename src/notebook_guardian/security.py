...
def _is_potential_typosquat(package_name: str) -> bool:
    ...
    for legit in legitimate_packages:
        ...
        if (package_lower != legit and 
            (package_lower.replace('-', '') == legit.replace('-', '') or
             package_lower.replace('_', '') == legit.replace('_', '') or
             _levenshtein_distance(package_lower, legit) == 1)):
            return True
...
def create_security_report(packages: List[str]) -> dict:
    ...
    if report['suspicious_packages']:
        report['warnings'].append(
            f"{len(report['suspicious_packages'])} packages flagged as suspicious"
        )
    
    if report['blocked_packages']:
        report['warnings'].append(
            f"{len(report['blocked_packages'])} packages blocked by security policy"
        )
