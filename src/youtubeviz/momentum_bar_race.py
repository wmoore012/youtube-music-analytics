...
def create_momentum_bar_race(
    ...
    if df.empty:
        return go.Figure().add_annotation(text="No data available for momentum bar race", x=0.5, y=0.5, showarrow=False)

    # Calculate momentum index
    momentum_df = calculate_momentum_index(df, artist_col=artist_col)

    if momentum_df.empty:
        return go.Figure().add_annotation(text="Insufficient data to calculate momentum", x=0.5, y=0.5, showarrow=False)

    # Group by week and artist, get average momentum per week
    weekly_momentum = momentum_df.groupby(["week_start", artist_col])["momentum_index"].mean().reset_index()
    ...
    if artists and weeks:
        full_index = pd.MultiIndex.from_product([weeks, artists], names=["week_start", artist_col]).to_frame(
            index=False
        )
        weekly_momentum = full_index.merge(weekly_momentum, on=["week_start", artist_col], how="left").sort_values(
            ["week_start", "momentum_index"], ascending=[True, True]
        )
    ...
    for week in weeks:
        ...
        frame = go.Frame(
            ...
            layout=go.Layout(title_text=f"Artist Momentum Rankings - {week.strftime('%b %d, %Y')}"),
        )
        frames.append(frame)
    ...
    if breakout_periods_sorted:
        breakout_summary = ", ".join(
            [f"{artist} ({date.strftime('%b %Y')})" for artist, date, _ in breakout_periods_sorted]
        )
        title_text = (
    ...
    fig.update_layout(
        title=title_text,
        xaxis=dict(title="Momentum Index (0-100)", range=[0, 105]),
        yaxis=dict(title="Artist", categoryorder="total ascending"),
        height=600,
    ...
    fig.add_vline(x=breakout_threshold, line_dash="dash", line_color="red", line_width=2)
