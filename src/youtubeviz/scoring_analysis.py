...
class ScoringAnalyzer:
    ...
    def prepare_momentum_data(self, videos_df: pd.DataFrame, metrics_df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_cols = ['artist_name', 'video_id', 'published_at', 'view_count', 
                        'like_count', 'comment_count', 'channel_title', 'metrics_date']
    ...
    def prepare_engagement_data(self, videos_df: pd.DataFrame, metrics_df: pd.DataFrame, 
    ...
    def calculate_real_scores(self, videos_df: pd.DataFrame, metrics_df: pd.DataFrame, 
                            sentiment_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
...
def create_momentum_scores_chart(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty:
        return go.Figure().add_annotation(text="No momentum data available", 
...
def create_engagement_distribution_chart(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty:
        return go.Figure().add_annotation(text="No engagement data available",
                                        x=0.5, y=0.5, showarrow=False)
...
@bulletproof_chart("Scoring System Performance", ["algorithm_name", "total_runs", "overall_avg_score"], timeout_sec=10.0)
def create_scoring_performance_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(x=df['algorithm_name'], y=df['total_runs'], name='Total Runs',
               marker_color='#636EFA'),
        row=1, col=1
    )
    ...
    # Average scores
    fig.add_trace(
        go.Bar(x=df['algorithm_name'], y=df['overall_avg_score'], name='Avg Score',
               marker_color='#EF553B'),
        row=1, col=2
    )
    ...
    # Total results
    fig.add_trace(
        go.Bar(x=df['algorithm_name'], y=df['total_results'], name='Total Results',
               marker_color='#00CC96'),
        row=2, col=1
    )
    ...
    if 'last_run' in df.columns:
        fig.add_trace(
            go.Scatter(x=df['last_run'], y=df['overall_avg_score'], 
                      mode='markers+lines', name='Score Trend',
                      marker_color='#AB63FA'),
            row=2, col=2
        )
...
def create_artist_score_radar(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty or len(df) == 0:
        return go.Figure().add_annotation(text="No artist data available",
                                        x=0.5, y=0.5, showarrow=False)
