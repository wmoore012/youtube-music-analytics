...
def detect_performance_patterns(df: pd.DataFrame) -> Dict[str, List[str]]:
    ...
    for artist in artist_metrics["artist_name"]:
        ...
        if len(artist_data) >= config["min_data_points"]:
            ...
            if first_half_avg > 0:
                ...
                if growth_rate > config["growth_threshold"]:
                    patterns["growth_artists"].append(artist)
                elif growth_rate < -config["growth_threshold"]:
                    patterns["declining_artists"].append(artist)
                else:
                    patterns["stable_artists"].append(artist)

    # Identify top performers by total views
    top_percentile = np.percentile(artist_metrics["total_views"], config["top_performer_percentile"])
    ...
    patterns["engagement_leaders"] = artist_metrics[
        artist_metrics["avg_engagement"] >= config["engagement_threshold"] / 100
    ]["artist_name"].tolist()
...
**Data Period**: {performance_df['date'].min().strftime('%Y-%m-%d')} to {performance_df['date'].max().strftime('%Y-%m-%d')}
...
def generate_compassionate_insights(
    ...
    if patterns["declining_artists"]:
        insights.append(
            ...
            "Every artist goes through different phases - let's explore new creative directions and fan engagement strategies."
        )
    ...
    insights.append(
        "🎯 **Remember**: Every artist's journey is unique. These insights help us support each artist's individual path to success."
    )
