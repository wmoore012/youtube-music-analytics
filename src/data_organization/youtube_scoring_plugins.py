...
from youtubeviz.data import load_artist_daily_metrics, load_sentiment_daily, _get_engine
...
class ArtistMomentumScoringPlugin(ScoringPlugin):
    ...
    def calculate_scores(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for artist_name, artist_data in data.groupby("artist_name"):
            ...
            if pd.isna(artist_name) or artist_name is None or str(artist_name).strip() == "" or str(artist_name).lower() == "none":
                continue
...
class GrowthPotentialScoringPlugin(ScoringPlugin):
    ...
    def calculate_scores(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for artist_name, artist_data in data.groupby("artist_name"):
            ...
            daily_metrics = trend_data.groupby("metrics_date").agg({
                "view_count": "sum",
                "like_count": "sum", 
                "comment_count": "sum"
            }).reset_index().sort_values("metrics_date")
