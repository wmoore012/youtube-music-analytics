...
from src.data_organization.open_source_plugin_framework import OpenSourceScoringPlugin, PluginMetadata
...
class ViewVelocityPlugin(OpenSourceScoringPlugin):
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            input_requirements=["video_id", "view_count", "published_date", "analytics_date"],
            output_schema={
                "video_id": "object",
    ...
    def calculate_scores(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for video_id, video_data in data.groupby("video_id"):
                ...
                video_data["days_diff"] = (video_data["analytics_date"] - video_data["analytics_date"].shift(1)).dt.days
                ...
                if current_views < min_views:
                    ...
                    velocity_score = velocity_weight * velocity_component + acceleration_weight * max(
                        acceleration_component, 0
                    )
...
class EngagementQualityPlugin(OpenSourceScoringPlugin):
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            input_requirements=["video_id", "view_count", "like_count", "comment_count"],
            output_schema={
                "video_id": "object",
...
class CrossPlatformMomentumPlugin(OpenSourceScoringPlugin):
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            input_requirements=["entity_id", "metric_value", "metric_date", "platform"],
            output_schema={
                "entity_id": "object",
        ...
        try:
            ...
            for entity_id, entity_data in data.groupby("entity_id"):
                ...
                if normalize_platforms:
                    normalized_values = self._normalize_by_platform(entity_data["metric_value"], platform)
                else:
...
class GenreSpecificScoringPlugin(OpenSourceScoringPlugin):
    ...
    def get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            input_requirements=["entity_id", "genre", "view_count", "like_count", "comment_count"],
            output_schema={
                "entity_id": "object",
