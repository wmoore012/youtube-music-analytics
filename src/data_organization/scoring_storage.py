        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                self._store_entity_scores(
                    conn, run_id, algorithm_id, result.entity_scores, 
                    entity_type, result.calculation_timestamp
                )
        ...
        for _, row in scores_df.iterrows():
            ...
            result_record = {
                ...
                "metadata": json.dumps({k: v for k, v in row.items() 
                                      if k not in ["entity_id", "score_value", "confidence"]})
            }
