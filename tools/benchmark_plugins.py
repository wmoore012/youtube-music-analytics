        ...
        if unique_scores > 1:
            ...
            if len(scores) > 3:
                ...
                for i, (_, row) in enumerate(bottom_artists.iterrows(), 1):
                    print(f"   #{len(scores)-len(bottom_artists)+i} {row['entity_id']}: {row['score_value']:.4f} ({row['momentum_category']})")
        else:
...
def benchmark_engagement_scoring():
    ...
    try:
        ...
        print(f"   Score range: {scores['score_value'].min():.4f} - {scores['score_value'].max():.4f}")
        print(f"   Engagement rate range: {scores['engagement_rate'].min():.6f} - {scores['engagement_rate'].max():.6f}")
        print(f"   Avg confidence: {scores['confidence'].mean():.4f}")
...
def benchmark_scoring_engine_integration():
    ...
    try:
        ...
        start_time = time.time()
        latest_scores = engine.get_latest_scores(
