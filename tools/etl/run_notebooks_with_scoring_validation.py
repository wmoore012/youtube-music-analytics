...
class NotebookScoringValidator:
    ...
    def run_complete_notebook_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   Real Data Confirmed: {'✅' if any(v.get('real_data_found') for v in self.results['scoring_validations'].values()) else '❌'}"
            )
            print(
                f"   Charts Generated: {'✅' if any(v.get('charts_generated') for v in self.results['scoring_validations'].values()) else '❌'}"
            )
