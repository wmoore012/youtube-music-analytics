...
handles cases where what we think is the artist part might actually be part of the title.
 The parser now also recognizes possessive forms like "Artist's song Title" and correctly extracts the artist name and song title. It also handles titles with "with the label" phrases
...
LEGITIMATE_ARTIST_CHANNELS = [r"Emanny Music",  # Validated as Emanny's official channel r".*VEVO$",  # All VEVO channels are legitimate r".*Official$",  # Official channels are legitimate
                               ]
...
# First, try to extract version from parentheses or brackets for pattern in [r"\(\s*(.*?)\s*\)", r"\[\s*(.*?)\s*\]"]:
        matches = re.findall(pattern, title)
        for match in matches:
            # Check if the match contains a version indicator
            # Skip default "Audio" type
            version_type = extract_version_type(match, channel_name) if version_type and version_type != "Audio":
                # Remove the version type from the title
                title = re.sub(pattern.replace("(.*?)", re.escape(match)), "",
                                title,
                                flags=re.IGNORECASE,
                                )
...
for separator in SEPARATORS:
        if separator in title:
            ...
            if len(parts) == 2:
                # Skip default "Audio" type
                version_type = extract_version_type(parts[1], channel_name) if version_type and version_type != "Audio":
...
for separator in SEPARATORS:
        if separator in title:
            ...
            if len(parts) == 2:
                ...
                # Check if it's not just a single word (likely not an artist) if " " in potential_artist or (channel_name and potential_artist.lower() in channel_name.lower()):
...
if not artists and channel_name:
        # Remove common channel suffixes channel = re.sub(             r"VEVO$|Official$|Music$|Records$|Recordings$", "",  # noqa: E128
            channel_name,  # noqa: E128
            flags=re.IGNORECASE,  # noqa: E128
        ).strip()
        if channel:
...
# More comprehensive regex for featuring clauses # Handles variations like "ft.", "feat.", "featuring", "ft", etc.
...
# Handle special cases where artists are separated by more complex patterns # Example: "Lute featuring Blakk Soul & Ari Lennox" should split into ["Lute", "Blakk Soul", "Ari Lennox"]

# First check for featuring patterns and handle them specially featuring_match = re.search(r"(.+?)\s+(?:feat\.?|featuring|ft\.?)\s+(.+)", cleaned_block, re.IGNORECASE)
if featuring_match:
...
for part in parts:
        ...
        if (
            len(part) > 50  # Too long to be an artist name or part.lower() in ["official", "music", "video", "hd", "hq", "audio", "lyrics", "vevo"] or re.match(r"^\d+$", part)
        ):  # Just numbers
            continue
...
def _extract_collaboration_clauses(title: str) -> Tuple[str, List[str]]: """Remove featuring / with clauses and collect collaborator names.
...
while True:
 ...
 # Collaborators embedded in a feat. clause sometimes use "with" # inside the captured text ("feat. Guest One with Crew Two"). We # split on "with" only in that situation to avoid breaking titles # such as "Sleep With The Light On" that reach this helper via the
            # dedicated _RX_WITH_CLAUSE pattern. if pattern is _RX_FEAT_CLAUSE and re.search(r"\bwith\b", captured_names, re.I): segments = re.split(r"\bwith\b", captured_names, flags=re.I)
...
for pattern in MEANINGLESS_DESCRIPTORS: cleaned_title = re.sub(pattern, "", cleaned_title, flags=re.IGNORECASE)

# Clean up extra spaces and empty parentheses / brackets cleaned_title = re.sub(r"\s+", " ", cleaned_title).strip() cleaned_title = re.sub(r"\(\s*\)", "", cleaned_title)  # Empty parentheses cleaned_title = re.sub(r"\[\s*\]", "", cleaned_title)  # Empty brackets

return cleaned_title


def _extract_artist_from_title_start(title: str, channel_name: str = None) -> tuple[List[str], str]: """
Try to extract artist names from the beginning of a title, even if no separator is present. This handles cases like "JID, Lute Ma Boy Lyrics" where the artists are at the start. """ # First try the more specific pattern for "Artist1, Artist2 Title" comma_pattern = r"^([A-Za-z0-9\s&.']{{1,
        15}}),
        \s+([A-Za-z0-9\s&.']{{1,
        15}})\s+([A-Za-z0-9\s'""]{{3, }})(?:\s+[Ll]yrics?)?$"
...
artist_pattern = r"^([A-Za-z0-9\s&.,']{{1,50}})\s+([A-Z][A-Za-z0-9\s'""]{{3,}})(?:\s+[Ll]yrics?)?$"
...
if match:
        ...
        # Check if this looks like a list of artists if "," in potential_artists_str or " and " in potential_artists_str.lower(): artists = _split_primary_block(potential_artists_str)             # Filter out single letters or very short "names" that might be noise
...
result = { "is_multi_song": False, "songs": [], "artist": None, "performance_type": None,
}
 # Pattern for quoted song lists: Artist "Song1, Song2 & Song3" quoted_pattern = r'(\w+(?:\s+\w+)*)\s*["""]([^"""]+)["""]'
...
if quoted_match:
        ...
        # Split songs by common separators song_separators = re.compile(r"\s*(?:,|&|\sand\s)\s*", re.IGNORECASE)
...
# 0️⃣ remove extraneous bracketed tags early (keeps feat./with clauses) cleaned = _RX_BRACKETS.sub("", raw_title).strip()
...
multi_song_info = _detect_multi_song_performance(cleaned) if multi_song_info["is_multi_song"]:
        ...
        if len(featured_songs) > 0: title_part = f"{primary_song} (with {', '.join(featured_songs)})"
        else:
...
if channel_title and _is_ripper_channel(channel_title): # Pattern: "Artist — Song | LIVE Performance | Broadcaster" live_pattern = r"^([A-Za-z\s&.\']+)\s*[—-]\s*([^|]+)(?:\s*\|\s * LIVE\s * Performance)?(?:\s*\|\s*(.+))?$"
        ...
        if live_match:
            ...
            # Clean up the song part song_part = re.sub(r"\s*\|\s * LIVE\s * Performance.*$", "", song_part, flags=re.IGNORECASE)

            return { "title": song_part.strip(), "primary": [artist], "featured": [], "version_type": "Live Performance", "broadcaster": broadcaster,
            }
...
if label_match:
 ...
 # If no artist information found but we have a "with the label" phrase,
        # treat the song_info as the title and ignore the label part return {"title": song_info.strip(" \"'"), "primary": [], "featured": []}
...
if quoted_match:
        ...
        if len(potential_artist.split()) <= 3 and not any( word.lower() in potential_artist.lower() for word in ["official", "music", "channel", "video"]
        ):
...
topic_artist = None if channel_title and channel_title.lower().endswith(" - topic"):
        topic_artist = channel_title[:-7].strip()
 # 2️⃣ split on first ' - '  (most common "Artist – Title" pattern) if " - " in cleaned: artist_part, title_part = map(str.strip, cleaned.split(" - ", 1))
...
if not primary_artists and channel_title and not _is_ripper_channel(channel_title):
        ...
        channel = re.sub( r"VEVO$|Official$|Music$|Records$|Recordings$", "",
            channel_title,
            flags=re.IGNORECASE,
        ).strip()
        ...
        if (
            ...
            and not any(
                ...
                for word in [ "official", "music", "records", "recordings", "channel", "vevo",
                ]
            )
        ):
...
# Check title for lyric indicators lyric_indicators = ["lyric", "lyrics", "with lyrics", "official lyrics"]
...
official_indicators = [ "official video", "official music video", "official mv", "official m / v",
]
for indicator in official_indicators:
        if indicator.lower() in title.lower():
            return True
 # Check if the channel name contains the artist name and has "VEVO" or "Official"
if channel_name: if "vevo" in channel_name.lower() or "official" in channel_name.lower():
            return True
...
# Get the video title and channel name title = video_data.get("title", "") channel_name = video_data.get("channel_name", "") description = video_data.get("description", "")
...
video_data.update(
        { "parsed_title": parsed["title"], "parsed_artists": ", ".join(parsed["primary"] + parsed["featured"]), "primary_artists": parsed["primary"], "featured_artists": parsed["featured"], "version_type": version_type,
        }
)
...
# Extract ISRC from the video data if available isrc = video_data.get("isrc")
if isrc:
        # Try to find a direct match by ISRC
        for song in songs: if song.get("ISRC") == isrc:
                return song

# If no ISRC match, try to match by title and artists parsed_title = video_data.get("parsed_title", "").lower() parsed_artists = [artist.lower() for artist in video_data.get("parsed_artists", [])]
...
matches = []
for song in songs: song_title = song.get("song_title", "").lower()
        ...
        artist_similarity = 0 if parsed_artists and "artists" in song: song_artists = [artist.lower() for artist in song.get("artists", [])]
...
# Normalise inputs early str1 = (str1 or "").lower().strip() str2 = (str2 or "").lower().strip()
...
for pattern in unicode_patterns:
        if re.search(pattern, title): return "Chopped and Screwed"

# Define version type patterns with their corresponding labels
VERSION_MAP = { r"\bofficial (music )?video\b": "Official Music Video", r"\bofficial audio\b": "Official Audio", r"\blyric(s)? video\b": "Lyric Video", r"\b(acoustic|unplugged)\b": "Acoustic", r"\blive( at| from)?\b": "Live Performance", r"\b(chopped.*screwed|slowed.*reverb)\b": "Chopped and Screwed", r"\b(remix|mashup)\b": "Remix", r"\bon the radar performance\b": "On The Radar Performance", r"\bcolors (show|performance)\b": "COLORS Performance", r"\bvevo dscvr\b": "VEVO DSCVR", r"\bperformance video\b": "Performance Video", r"\blive session\b": "Live Session",
}
...
# Check channel name for Topic channels (usually official audio) if channel_name and "- topic" in channel_name.lower(): return "Official Audio"

# Default to Audio if no specific version type is found return "Audio"
...
for i, title in enumerate(test_titles):
        ...
        parsed = parse_youtube_title(title, channel) print(f"Title: {title}") print(f"Channel: {channel}") print(f"Primary Artists: {parsed['primary']}") print(f"Featured Artists: {parsed['featured']}") print(f"Cleaned Title: {parsed['title']}")
        # Version type is not returned by parse_youtube_title, need to get it separately
        version_type = extract_version_type(title, channel) print(f"Version Type: {version_type}") print("---")
