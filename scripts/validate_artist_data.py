...
def load_expected_artists_from_config() -> Dict[str, Any]:
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            "config/expected_artists.json not found. "
            "This file is required for CI/CD validation."
        )
...
def main():
    ...
    parser.add_argument("--update-config", action="store_true", 
                       help="Update config file from current .env")
    parser.add_argument("--notebooks-only", action="store_true",
                       help="Only validate notebook outputs")
