...
from src.notebook_guardian.security import enable_safe_mode, create_security_report
from src.data_organization.notebook_validator import NotebookValidator, MetricExplainer
...
def create_validation_charts(validation_results, performance_data, security_data):
    ...
    fig.update_layout(
        height=1200,
        title_text="🛡️ Notebook Guardian Validation Results Dashboard",
        title_x=0.5,
        showlegend=True
    )
...
def create_performance_comparison_chart(performance_df):
    ...
    fig.update_layout(
        ...
        yaxis2=dict(
            title="Throughput (Lines of Code per Second)",
            side="right",
            overlaying="y",
            color="blue"
        ),
...
def create_security_analysis_chart(security_df):
    ...
    fig.add_trace(go.Bar(
        x=security_df['file'],
        y=security_df['blocked_packages'],
...
def generate_summary_report(validation_df, performance_df, security_df):
    ...
    print(f"🔒 Package Security: {safe_packages}/{total_packages} safe ({safe_packages/max(total_packages,1)*100:.1f}%)")
