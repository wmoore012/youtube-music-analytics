...
from src.data_organization.scoring_storage import AlgorithmNotRegisteredError, ScoringStorage, ScoringStorageError
...
class TestScoringStorageMocked:
        ...
        def test_real_entity_rankings(self):
            ...
            # Get rankings
            rankings = self.storage.get_entity_rankings(algorithm_name="artist_momentum_scorer", entity_type="artist")
        ...
        def test_real_scoring_history(self):
            ...
            history = self.storage.get_scoring_history(
                entity_id=real_artist, entity_type="artist", algorithm_name="artist_momentum_scorer"
            )
...
#     def setup_method(self):
        ...
        def test_register_algorithm_new(self):
            ...
            algorithm_id = self.storage.register_algorithm("test_algorithm", "1.0.0", "Test algorithm", "Test Author")
        ...
        def test_get_latest_scores_with_filters(self):
            ...
            with patch("pandas.read_sql", return_value=mock_df) as mock_read_sql:
                result = self.storage.get_latest_scores(
                    algorithm_name="test_scorer", entity_type="artist", entity_ids=["artist1", "artist2"], limit=10
                )
        ...
        def test_get_scoring_history(self):
            ...
            with patch("pandas.read_sql", return_value=mock_df) as mock_read_sql:
                result = self.storage.get_scoring_history(
                    entity_id="artist1", entity_type="artist", algorithm_name="test_scorer", days_back=30
                )
        ...
        def test_get_entity_rankings(self):
            ...
            with patch("pandas.read_sql", return_value=mock_df) as mock_read_sql:
                result = self.storage.get_entity_rankings(algorithm_name="test_scorer", entity_type="artist", limit=10)
        ...
        def test_store_result_with_complex_metadata(self):
            ...
            result = ScoringResult(
                ...
                metadata={
                    ...
                    "parameters": {"window_days": 30, "weights": {"growth": 0.4, "engagement": 0.6}},
...
class TestScoringStorageRealIntegration:
        ...
        def test_real_full_workflow_multiple_algorithms(self):
            ...
            try:
                ...
                momentum_data = artist_data.rename(
                    columns={
                        "date": "metrics_date",
                        "views": "view_count",
                        "likes": "like_count",
        ...
        def test_real_time_series_scoring_analysis(self):
            ...
            try:
                ...
                history = self.storage.get_scoring_history(
                    entity_id=top_artist, entity_type="artist", algorithm_name="artist_momentum_scorer", days_back=30
                )
...
class TestScoringStorageIntegration:
        ...
        def test_multiple_algorithm_storage(self):
            ...
            algorithms = [("momentum_scorer", "1.0.0"), ("engagement_scorer", "1.0.0"), ("growth_scorer", "1.0.0")]

            for alg_name, version in algorithms:
                scores_df = pd.DataFrame(
                    {"entity_id": ["artist1", "artist2"], "score_value": [0.8, 0.6], "confidence": [0.9, 0.7]}
                )

                result = ScoringResult(algorithm_name=alg_name, algorithm_version=version, entity_scores=scores_df)
