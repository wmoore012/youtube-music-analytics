...
engine = _prepare_engine(150) monkeypatch.delenv("ARTIST_ALIASES_JSON", raising=False)

summary = data_module.benchmark_run_artist_metrics_pipeline(engine=engine, iterations=3)
 assert summary["rows"] == 150 assert summary["iterations"] == 3 assert summary["duration_sec"] < 0.5
