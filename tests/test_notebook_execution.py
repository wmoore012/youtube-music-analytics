...
class TestNotebookExecution:
    ...
    def test_music_analytics_execution(self):
        ...
        result = subprocess.run([
            sys.executable, "execute_music_analytics.py"
        ...
        assert found_count == self.EXPECTED_ARTIST_COUNT, \
    ...
    def test_data_quality_execution(self):
        ...
        result = subprocess.run([
            sys.executable, "execute_data_quality.py"
        ], capture_output=True, text=True, timeout=120)
    ...
    def test_artist_comparison_execution(self):
        ...
        result = subprocess.run([
            sys.executable, "execute_artist_comparison.py"
        ], capture_output=True, text=True, timeout=120)
    ...
    def test_all_notebooks_artist_consistency(self):
        ...
        assert consistent_count == self.EXPECTED_ARTIST_COUNT, \
    ...
    def test_notebook_output_quality(self):
        ...
        assert any(char in output for char in ['$', 'USD', 'revenue']), \
            "No revenue information found in music analytics"
        
        # Should contain view counts
        assert re.search(r'\d{1,3}(,\d{3})*\s+views', output), \
            "No properly formatted view counts found"
        
        # Should contain percentages
        assert re.search(r'\d+\.\d+%', output), \
            "No percentage values found"
        ...
        assert "OVERALL DATA QUALITY SCORE" in output, \
            "No overall quality score found"
    ...
    def test_comprehensive_artist_validation_integration(self):
        ...
        assert "ALL VALIDATIONS PASSED" in output, \
            "Comprehensive artist validation did not pass"
        ...
        for component in validation_components:
            assert f"✅ PASSED: {component}" in output, \
                f"Component '{component}' did not pass validation"
