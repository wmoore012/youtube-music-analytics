...
def test_load_artist_daily_metrics_applies_alias_mapping(monkeypatch):
    ...
    assert not df.empty
    assert df.loc[0, "artist_name"] == "The Weeknd"
    assert df.loc[0, "views"] == 1000
    assert df.loc[0, "likes"] == 50


def test_run_artist_metrics_pipeline_returns_revenue(monkeypatch):
    engine = _setup_engine()
    monkeypatch.delenv("ARTIST_ALIASES_JSON", raising=False)

    result = data_module.run_artist_metrics_pipeline(engine=engine)

    assert set(result.keys()) == {"daily_metrics", "revenue"}
    daily = result["daily_metrics"]
    revenue = result["revenue"]
    assert not daily.empty
    assert not revenue.empty
    assert revenue.loc[0, "artist_name"] == "The Weeknd"
    assert revenue.loc[0, "est_revenue_usd"] == pytest.approx(3.0)
