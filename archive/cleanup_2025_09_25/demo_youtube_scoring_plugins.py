...
from youtubeviz.data import _get_engine, load_artist_daily_metrics
...
def load_sample_data_for_momentum(engine, limit_artists=5):
    ...
    top_artists = (
        data.groupby("artist_name")["views"].sum().sort_values(ascending=False).head(limit_artists).index.tolist()
    )
...
def load_sample_data_for_growth_potential(engine, limit_artists=3):
    ...
    data = pd.read_sql(text(query), engine, params={"start_date": start_date, "end_date": end_date})
...
def demo_momentum_scoring():
    ...
    try:
        ...
        print(
            scores_df[["entity_id", "score_value", "confidence", "momentum_category", "total_videos", "recent_videos"]]
            .head(10)
...
def demo_growth_potential_scoring():
    ...
    try:
        ...
        display_cols = ["entity_id", "score_value", "confidence", "trend_direction", "growth_velocity", "data_points"]
        print(scores_df[display_cols].head(10).to_string(index=False))
