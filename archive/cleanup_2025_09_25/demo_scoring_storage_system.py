...
def demo_storage_queries(run_id=None):
    ...
    try:
        ...
        latest_scores = storage.get_latest_scores(
            algorithm_name="artist_momentum_scorer", entity_type="artist", limit=10
        )

        if not latest_scores.empty:
            print(
                latest_scores[["entity_id", "score_value", "confidence_level", "calculation_timestamp"]].to_string(
                    index=False
                )
            )
        else:
        ...
        if not performance.empty:
            print(
                performance[["algorithm_name", "total_runs", "total_results", "overall_avg_score"]].to_string(
                    index=False
                )
            )
        else:
        ...
        # Get entity rankings
        print(f"\n🏆 Artist Rankings:")
        rankings = storage.get_entity_rankings(algorithm_name="artist_momentum_scorer", entity_type="artist", limit=5)
        ...
        if not latest_scores.empty:
            ...
            if not history.empty:
                print(
                    history[["calculation_timestamp", "score_value", "confidence_level"]].head().to_string(index=False)
                )
            else:
...
def demo_multiple_algorithms():
    ...
    try:
        ...
        for algorithm_name, data, entity_type in algorithms_data:
            ...
            result = engine.execute_scoring(algorithm_name, data, store_results=True, entity_type=entity_type)
