...
from youtubeviz.music_ml_classifier import create_transformer_models, MusicSentimentTransformer
...
def test_transformer_on_neutral_comments():
    ...
    problematic_comments = [
        ...
        "Bro go hard when he raps about God not when he talks about the devil fr fr", 
...
def test_specific_music_terms():
    ...
    try:
        ...
        for text, expected in music_slang_tests:
            ...
            print(f"   \"{text:20s}\" → {predicted.upper():8s} ({confidence:.3f}) | Expected: {expected.upper():8s} {status}")
