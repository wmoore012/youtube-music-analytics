...
## Data Ops Next Actions

1. **Chart storage schema** - Define normalized tables (snake_case, natural keys) for chart-ready metrics.
2. **Nightly ETL** - Add a scheduled workflow to refresh analytics tables every night.
3. **Chart wiring map** - Document which notebook outputs feed each frontend chart.
4. **Null audit + remediation** - ISRC is 100% null in `normalized_music_videos.csv`; decide whether to drop the column or fix ingestion and re-run ETL.
5. **Executed notebook outputs** - Re-run the Professional Dashboard notebook with papermill so `notebooks/executed/` contains chart outputs again.
